import sys
import unittest
import torch

from utils.non_max_suppression import non_max_suppression

# Credit : https://github.com/aladdinpersson/Machine-Learning-Collection/blob/master/ML_tests/Object_detection_tests/nms_test.py

class TestNonMaxSuppression:
    # test cases we want to run
    t1_boxes = [
        [1, 1, 0.5, 0.45, 0.4, 0.5],
        [1, 0.8, 0.5, 0.5, 0.2, 0.4],
        [1, 0.7, 0.25, 0.35, 0.3, 0.1],
        [1, 0.05, 0.1, 0.1, 0.1, 0.1],
    ]

    c1_boxes = [[1, 1, 0.5, 0.45, 0.4, 0.5], [1, 0.7, 0.25, 0.35, 0.3, 0.1]]

    t2_boxes = [
        [1, 1, 0.5, 0.45, 0.4, 0.5],
        [2, 0.9, 0.5, 0.5, 0.2, 0.4],
        [1, 0.8, 0.25, 0.35, 0.3, 0.1],
        [1, 0.05, 0.1, 0.1, 0.1, 0.1],
    ]

    c2_boxes = [
        [1, 1, 0.5, 0.45, 0.4, 0.5],
        [2, 0.9, 0.5, 0.5, 0.2, 0.4],
        [1, 0.8, 0.25, 0.35, 0.3, 0.1],
    ]

    t3_boxes = [
        [1, 0.9, 0.5, 0.45, 0.4, 0.5],
        [1, 1, 0.5, 0.5, 0.2, 0.4],
        [2, 0.8, 0.25, 0.35, 0.3, 0.1],
        [1, 0.05, 0.1, 0.1, 0.1, 0.1],
    ]

    c3_boxes = [[1, 1, 0.5, 0.5, 0.2, 0.4], [2, 0.8, 0.25, 0.35, 0.3, 0.1]]

    t4_boxes = [
        [1, 0.9, 0.5, 0.45, 0.4, 0.5],
        [1, 1, 0.5, 0.5, 0.2, 0.4],
        [1, 0.8, 0.25, 0.35, 0.3, 0.1],
        [1, 0.05, 0.1, 0.1, 0.1, 0.1],
    ]

    c4_boxes = [
        [1, 0.9, 0.5, 0.45, 0.4, 0.5],
        [1, 1, 0.5, 0.5, 0.2, 0.4],
        [1, 0.8, 0.25, 0.35, 0.3, 0.1],
    ]

    def test_remove_on_iou(self):
        bboxes = non_max_suppression(
            self.t1_boxes,
            threshold=0.2,
            iou_threshold=7 / 20,
            format_boxes="midpoint",
        )
        assert sorted(bboxes) == sorted(self.c1_boxes)

    def test_keep_on_class(self):
        bboxes = non_max_suppression(
            self.t2_boxes,
            threshold=0.2,
            iou_threshold=7 / 20,
            format_boxes="midpoint",
        )
        assert sorted(bboxes) == sorted(self.c2_boxes)

    def test_remove_on_iou_and_class(self):
        bboxes = non_max_suppression(
            self.t3_boxes,
            threshold=0.2,
            iou_threshold=7 / 20,
            format_boxes="midpoint",
        )
        assert sorted(bboxes) == sorted(self.c3_boxes)

    def test_keep_on_iou(self):
        bboxes = non_max_suppression(
            self.t4_boxes,
            threshold=0.2,
            iou_threshold=9 / 20,
            format_boxes="midpoint",
        )
        assert sorted(bboxes) == sorted(self.c4_boxes)